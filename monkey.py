...
class Main:
    def __init__(self):
        ...
        if REASON.upper() in ('1', 'ILLEGAL CONTENT'):
            ...
            print('\n[?] Reason invalid.')
            os.system(
                'title [Monkey Mass Reporter] - Restart Required - Press Any Key &&'
                'pause >NUL &&'
                'title [Monkey Mass Reporter] - Exiting - Please Wait 3 Seconds'
            )
        ...
        self.RESPONSES = {
            '401: Unauthorized': '[?] Invalid Monkey Token.',
    ...
    def _update_title(self):
        while True:
            os.system(f'title [Monkey Mass Reporter] - Monkey Sent: {self.sent} ^| Monkey Errors: {self.errors}')
            time.sleep(0.1)
    ...
    def setup(self):
        recognized = None
        if os.path.exists(config_json := 'settings.json'):
...
if __name__ == '__main__':
    os.system('cls && title [Monkey Mass Reporter] - Setup Screen')
    main = Main()
    main.setup()
